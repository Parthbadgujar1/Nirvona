...
abstract class BaseService
{
    ...
    protected function executeWithFallback(
        ...
        try {
            ...
            // A ServiceException is always deliberately thrown with a
            // specific, human-readable, safe-to-show message ("Answer
            // key not found for exam: X", "Student is already
            // registered for this exam", ...) - when the caller hasn't
            // supplied its own fallback, use that real message instead
            // of discarding it for the generic "Service temporarily
            // unavailable" text below. That generic fallback existing
            // at all is right (an *unexpected* crash, caught below,
            // must never leak raw internals to the client) but reusing
            // it here turned every ordinary validation/not-found error
            // across ~35 call sites into an indistinguishable, scary,
