...
class ExamService extends BaseService
{
        ...
        return $this->executeWithFallback(
            function () use ($data) {
                ...
                $exam = $this->examRepository->create([
                    ...
                    // Was hardcoded to 'scheduled' regardless of what the
                    // caller sent - the admin exam form's "Status" field
                    // (defaulting to "Draft") had no effect at all: every
                    // newly created exam went live to candidates
                    // immediately instead of staying a draft.
                    'status' => $data['status'] ?? 'draft',
                    'centreId' => $data['centreId'] ?? null,
                    'syllabusScope' => $data['syllabusScope'] ?? null,
    ...
    /** Fields the admin exam form is allowed to change. */
    private const EDITABLE_FIELDS = [
        'name', 'courseSlug', 'date', 'reportingTime', 'examTime', 'durationMinutes',
        'totalQuestions', 'totalMarks', 'centreId', 'status', 'instructions', 'syllabusScope',
    ];

    /**
     * Update an existing exam (edit form save, or a status-only change
     * like publish/close) - nothing previously let either happen
     * server-side, so both silently only mutated local frontend state.
     *
     * @param string $examId
     * @param array $data Any subset of EDITABLE_FIELDS
     * @return array
     */
    public function updateExam(string $examId, array $data): array
    {
        return $this->executeWithFallback(
            function () use ($examId, $data) {
                $existing = $this->examRepository->getById($examId);
                if (!$existing) {
                    throw new ServiceException("Exam not found: {$examId}", 'ExamService', false);
                }

                $updates = array_intersect_key($data, array_flip(self::EDITABLE_FIELDS));
                if (empty($updates)) {
                    throw new ServiceException("No editable fields provided", 'ExamService', false);
                }

                $this->examRepository->update($examId, $updates);

                $this->auditLog('UPDATE', 'Exam', $examId, array_keys($updates));

                return [
                    'success' => true,
                    'data' => $this->examRepository->getById($examId),
