...
export function useAsync<T>(
  ...
  return {
    ...
    status: state.status,
    error: state.error,
    // Bumping the nonce alone re-runs the fetch effect, but `state`
    // itself stays whatever it was (usually "error") until that fetch
    // resolves - so clicking "Try again" produced no visible change at
    // all until the request finished, and if it failed again the UI
    // looked frozen/broken rather than having visibly retried. Resetting
    // to "loading" immediately makes the retry observable every time.
