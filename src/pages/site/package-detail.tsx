...
export default function PackageDetailPage() {
  ...
  // Previously two sequential `await`s in a Next.js server component
  // (fetch the package, then its course's sibling durations); client-
  // side here via useAsync, the same pattern every other data-fetching
  // page in this app uses. The siblings fetch waits on the package's
  // courseSlug, so it's keyed off `pkg?.courseSlug` and returns an
  ...
  // Course catalogue copy stays on the mock data (see
  // catalogue.service.ts) - falls back to the not-found route if a
  // real package's courseSlug has no matching entry there yet.
  if (!pkg || !course) {
    return <Navigate to="/404" replace />;
  }

  const siblings = (siblingsAsync.data ?? []).filter((p) => p.id !== pkg.id);
