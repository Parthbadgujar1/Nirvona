...
/**
 * Real exams frequently have no custom `instructions` set (the column
 * is nullable TEXT - one newline-separated string, not an array - and
 * rarely populated). A printed admit card still needs some exam-day
 * guidance, so fall back to these standard ones rather than leaving
 * the section blank.
 */
const DEFAULT_INSTRUCTIONS = [
  "Arrive at the centre at least 30 minutes before the reporting time.",
  "Carry this admit card and one original government photo ID.",
  "Electronic devices, including phones and smartwatches, are not permitted in the exam hall.",
  "Your exam login works only on the allotted centre machine.",
];
      ...
      <div className="grid gap-0 border-b border-ink-200 sm:grid-cols-2 sm:divide-x sm:divide-ink-200">
        ...
        <section className="border-t border-ink-200 p-6 sm:border-t-0 sm:p-8">
          ...
          <dl className="mt-4 space-y-3">
            ...
            <Row label="Reporting time" value={exam.reportingTime || "To be announced"} />
            <Row label="Examination time" value={exam.examTime || "To be announced"} />
            <Row
              label="Duration"
              value={exam.durationMinutes ? `${exam.durationMinutes} minutes` : "To be announced"}
            />
            <Row
              label="Pattern"
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-[2fr_1fr]">
          ...
          <div className="rounded-xl bg-canvas p-4">
            ...
            <p className="mt-1 font-display text-2xl font-extrabold tabular text-navy-900">
              {admitCard.seatNo || "To be allocated"}
            </p>
            {/* Seat allocation happens at registration and isn't
                guaranteed by the time a card is generated - real data
                can have a null seatNo here, unlike the AdmitCard type's
                non-nullable `string` declaration suggests. */}
            {admitCard.seatNo && (
      ...
      <section className="p-6 sm:p-8">
        ...
        <ol className="mt-4 grid gap-2.5 sm:grid-cols-2">
          {(() => {
            ...
            return lines.length ? lines : DEFAULT_INSTRUCTIONS;
