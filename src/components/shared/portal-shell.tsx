...
export function PortalShell({
  ...
  // With the real backend wired up, an unauthenticated visitor gets a
  // null session (see useSession) instead of a fabricated one - send
  // them to sign in rather than crashing on session.name below. Waits
  // for `hydrated` so a real logged-in user's session (read from
  // localStorage after mount) isn't mistaken for "not signed in" on the
  // very first render.
  React.useEffect(() => {
    if (hydrated && !session) {
