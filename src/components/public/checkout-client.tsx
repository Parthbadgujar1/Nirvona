...
export function CheckoutClient() {
  ...
  // Real package (real id, real price) from the backend - was a
  // synchronous lookup against the frontend's mock package array, so
  // clicking through from anywhere that only had a mock id (or typing
  // one) always resolved to "No package selected" or, worse, a fake
  // package whose id the backend had never heard of (the actual cause
  // behind checkout's "API Error: Bad Request" / "Payment gateway is
  // experiencing issues" failures).
  const packageResult = useAsync(
  ...
  // component" error is about - it looks like it works most of the
  ...
  if (packageId && packageResult.status === "loading") {
    return <div className="container-nv py-20" aria-busy="true" />;
  }
          ...
          <div className="space-y-6">
            <Card className="p-6">
              ...
              <dl className="mt-5 grid gap-4 sm:grid-cols-2">
                {[
                  { label: "Full name", value: profile.data?.fullName ?? session?.name ?? "—" },
                  { label: "Student ID", value: session?.id ?? "—" },
                  { label: "Email", value: profile.data?.email ?? session?.email ?? "—" },
                  { label: "Mobile", value: profile.data?.mobile ?? "—" },
                  { label: "Class", value: profile.data?.className ?? "—" },
                  {
                    label: "City",
                    value:
