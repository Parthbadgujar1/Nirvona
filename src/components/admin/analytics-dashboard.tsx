...
export function AnalyticsDashboard() {
  ...
  // there is also no bare /admin/results (a result only exists scoped
  ...
  if (exams.status === "loading" || !exams.data || !examId || results.status === "loading" || !results.data) {
    return <LoadingState label="Building analytics" />;
  }
  ...
  if (results.data.length === 0) {
    return (
      <div className="space-y-6">
        <PageHeader
          title="Analytics"
          description="Score distribution, subject comparison and rank bands for an examination."
        />
        <EmptyState
          icon={Award}
          title="No results yet for this examination"
          description="Analytics populate once responses have been evaluated for this exam."
          action={{ label: "Import responses", href: "/admin/responses" }}
        />
      </div>
    );
  }
