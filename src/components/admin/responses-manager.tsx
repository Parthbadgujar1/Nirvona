      ...
      cell: (row) => (
        <div className="flex justify-end gap-2">
          ...
          <Button
            ...
            size="xs"
            onClick={async () => {
              toast.info(`Evaluating ${row.examId}`, {
                description: "Scoring every response against the current answer key.",
              });
              try {
                const result = await adminService.evaluateExam(row.examId);
                uploads.reload();
                toast.success(`${row.examId} evaluated`, {
                  description:
                    result.failed.length > 0
                      ? `${formatNumber(result.evaluated)} scored, ${formatNumber(result.failed.length)} failed (likely no answer key set yet).`
                      : `${formatNumber(result.evaluated)} response(s) scored.`,
                });
              } catch (error) {
                toast.error(error instanceof Error ? error.message : "Could not evaluate this exam.");
              }
            }}
          >
            <RefreshCcw />
