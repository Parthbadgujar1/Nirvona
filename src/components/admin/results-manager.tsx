...
/**
 * Was a fully static array (`done: true/false` never changed
 * regardless of which exam was selected or its real state). A `Result`
 * row only ever exists in this backend once the whole scoring pipeline
 * (responses -> answer key -> evaluation -> subject score -> percentile
 * -> rank) has run for that student, so "any results exist yet" is a
 * real, if coarse, proxy for stages 1-6 all having completed together.
 */
function buildWorkflow(hasResults: boolean, allPublished: boolean) {
  return [
    { title: "Student Responses", detail: "Imported and validated", icon: FileSpreadsheet, done: hasResults },
    { title: "Answer Key", detail: "Validated and published", icon: FileCheck2, done: hasResults },
    { title: "Evaluation", detail: "Scored per marking scheme", icon: CheckCircle2, done: hasResults },
    { title: "Subject Score", detail: "Section-wise breakdown", icon: Award, done: hasResults },
    { title: "Percentile", detail: "Normalised across cohort", icon: Percent, done: hasResults },
    { title: "Rank", detail: "All-India rank assigned", icon: Trophy, done: hasResults },
    { title: "Performance Analysis", detail: "Topic diagnostics generated", icon: Users, done: hasResults },
    { title: "Publish Result", detail: "Released to candidates", icon: Send, done: allPublished },
  ];
}

export function ResultsManager() {
  const exams = useAsync(() => adminService.exams(), []);
  // Was hard-coded to "CBT-03", a mock-era id matching no real exam -
  // there's also no bare /admin/results (a result only exists scoped
  // to one exam), so this page's data fetch 404'd unconditionally.
  // Empty until real exams load, then defaults to the first one.
  ...
  const results = useAsync(
    () => (examId ? adminService.resultsForExam(examId) : Promise.resolve([])),
    [examId],
  );

  if (exams.status === "error" || results.status === "error") {
    return <ErrorState onRetry={() => { exams.reload(); results.reload(); }} />;
  }
  if (exams.status === "loading" || !exams.data || !examId || results.status === "loading" || !results.data) {
  ...
  const totalCandidates = exam?.candidates ?? 0;
  const allPublished = rows.length > 0 && publishedCount === rows.length;
  const workflow = buildWorkflow(rows.length > 0, allPublished);
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog
        ...
        description="Publishing releases scores, ranks, percentiles and performance analysis to every candidate for this examination, and sends a result notification. This is visible to students immediately."
        confirmLabel="Publish results"
        onConfirm={async () => {
          try {
            await adminService.publishExamResults(examId);
            try {
              await adminService.sendNotification({
                title: "Result published",
                message: `Your ${exam?.name ?? examId} result is now available.`,
                type: "result",
                channels: ["whatsapp", "sms", "email", "portal"],
                examId,
              });
            } catch {
              // Publishing the results themselves already succeeded -
              // a notification-send failure shouldn't undo that or
              // block the flow, just skip the extra toast below.
            }
            setSelected([]);
            results.reload();
            toast.success(`${formatNumber(rows.length)} result(s) published`, {
              description: "Result notifications have been queued on WhatsApp, SMS and email.",
            });
          } catch (error) {
            toast.error(error instanceof Error ? error.message : "Could not publish these results.");
            throw error;
          }
        }}
        details={
          <div className="rounded-xl border border-ink-200 bg-canvas p-4 text-sm">
            ...
            <div className="mt-1.5 flex justify-between">
              ...
              <span className="font-semibold tabular text-navy-900">{formatNumber(rows.length)}</span>
            </div>
            <div className="mt-1.5 flex justify-between">
