...
// No generateStaticParams: package ids are real backend UUIDs now, not
// a fixed list known at build time. Next.js renders these on demand
// (dynamicParams defaults to true) instead of pre-generating a static
// set that would immediately go stale against the real database.
...
export async function generateMetadata({
  ...
  const pkg = await catalogueService.getPackage(id);
...
export default async function PackageDetailPage({
  ...
  // catalogue.service.ts) - falls back to notFound() if a real
  // package's courseSlug has no matching entry there yet.
  const course = getCourse(pkg.courseSlug);
  if (!course) notFound();

  const siblings = (await catalogueService.packagesForCourse(pkg.courseSlug)).filter(
    (p) => p.id !== pkg.id,
  );
