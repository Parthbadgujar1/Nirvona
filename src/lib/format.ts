...
/**
 * `new Date(null)` parses as the epoch (1 Jan 1970), not `Invalid
 * Date` - so a genuinely-empty date silently rendered as "01 Jan
 * 1970" instead of tripping the NaN check below. Real backend rows
 * commonly have null for optional date columns (dateOfBirth,
 * reportingTime, generatedAt, ...) even where the frontend type
 * declares them as a non-nullable `string`, so every caller needs
 * this guard, not just the ones that remembered to check first.
 */
